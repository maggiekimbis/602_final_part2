...
Original file is located at
    https://colab.research.google.com/drive/1tYuYPz9UhvBIKsO3bpv702UPPjmSod5Y
"""
...
def fetch_stock_data(symbol, api_key, year_needed):
    ...
    df = pd.DataFrame(df)
    # Using datetime.strptime to convert timestamp column
    df["timestamp"] = df["timestamp"].apply(lambda x: dt.datetime.strptime(x, '%Y-%m-%d'))
    df = df[df["timestamp"].dt.year >= year_needed]
    if symbol == "NTDOY":
        share_split_date = dt.datetime.strptime('2022-10-04', '%Y-%m-%d')  # Using datetime.strptime
